...
class MainCanvas extends Canvas implements Runnable {
	...
	public void falseHeroMove(int action) {
		if (falseallFlag == 0) {
			falseHeroCurrentImg = heroAllImage[action][1];
			falseallFlag = 1;
		} else if (falseallFlag == 1) {
			falseHeroCurrentImg = heroAllImage[action][0];
			falseallFlag = 2;
		} else if (falseallFlag == 2) {
			falseHeroCurrentImg = heroAllImage[action][1];
			falseallFlag = 3;
		} else if (falseallFlag == 3) {
			falseHeroCurrentImg = heroAllImage[action][2];
			falseallFlag = 0;
		}
